...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  const { role, setRole, darkMode, toggleDarkMode, commandPanelOpen, setCommandPanelOpen } = useAppContext()
  const { data: agentStatuses } = useApi<AgentStatus[]>('/agents/status', [])

  return (
    <div className="min-h-screen bg-gray-50/80 dark:bg-gray-950 flex">
      {/* Left Sidebar */}
      <aside className="w-60 bg-white dark:bg-gray-900/95 border-r border-gray-100 dark:border-gray-800/60 flex flex-col fixed h-full z-20">
        {/* Logo */}
        <div className="px-5 py-5 border-b border-gray-100 dark:border-gray-800/60">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-gradient-to-br from-kato-500 to-kato-700 rounded-xl flex items-center justify-center text-white font-bold text-base shadow-sm">K</div>
            <div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          ...
          {/* Interns */}
          <div className="pt-5 mt-4 border-t border-gray-100 dark:border-gray-800/60">
              ...
              {agentStatuses?.map(agent => {
                ...
                return (
                  <Link
                    key={agent.id}
                    to="/agents"
                    className="flex items-center gap-2.5 px-3 py-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800/60 transition-all group"
                  >
        ...
        <div className="px-3 py-3 border-t border-gray-100 dark:border-gray-800/60 space-y-2.5">
          ...
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-1.5 rounded-lg text-xs text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800/60 transition-all"
          >
      ...
      {/* Main */}
      <main className={`flex-1 ml-60 transition-all duration-300 ${commandPanelOpen ? 'mr-80' : 'mr-0'}`}>
      ...
      {!commandPanelOpen && (
        <button
          ...
          className="fixed right-5 bottom-5 z-30 w-11 h-11 bg-kato-600 hover:bg-kato-700 text-white rounded-2xl shadow-lg hover:shadow-xl flex items-center justify-center transition-all hover:-translate-y-0.5"
